...
// NOTE: There is no dedicated address-level geocode for the Cherokee Park Rd
// & Fish Creek Rd intersection itself, so this uses the ZIP 82084 (Tie
// Siding, WY) centroid, which sits on Cherokee Park Rd (County Road 31) in
// far southern Albany County, WY - the same rural crossroads area.
const localLat = 41.0803;
const localLon = -105.5075;
...
// Cherokee Park Rd corridor.
const wyomingGauges = [
    { id: "06659580", name: "Sand Creek at Colorado-Wyoming State Line", lat: 40.9937, lon: -105.7597, noaaId: "WYCO1" },
    { id: "06658500", name: "Laramie River Near Jelm, WY", lat: 41.0022, lon: -106.0142, noaaId: "WYCO2" }
];
        ...
        content: [
            {
                ...
                content: [
                    ...
                    { type: 'component', componentName: 'localForecast', title: '7-DAY GEOGRAPHIC SYNOPTIC OUTLOOK (82084)' }
                ]
            },
            {
                type: 'column',
                width: 30,
                content: [
                    { type: 'component', componentName: 'nwsAlerts', title: 'CRITICAL ENVIRONMENTAL SPECTRUM HAZARDS MATRIX - WYOMING' },
                    { type: 'component', componentName: 'hydrologyFeed', title: 'SAND CREEK / LARAMIE RIVER REAL-TIME STREAMFLOW' }
                ]
            },
            {
                type: 'column',
...
layout.registerComponent('radarMap', function(container) {
    container.getElement().html(`
        <div style="position:relative; width:100%; height:100%; background:#0d1117;">
            ...
            <iframe id="windyIframe" src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=radar&product=radar&level=surface&lat=41.0803&lon=-105.5075" style="width:100%; height:100%; border:none;"></iframe>
        </div>
    `);
    ...
    setTimeout(() => {
        ...
        select.on('change', function() {
            ...
            iframe.src = `https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=${layer}&product=${product}&level=surface&lat=41.0803&lon=-105.5075`;
        });
    }, 200);
});
...
layout.registerComponent('cloudMap', function(container) {
    container.getElement().html(`
        <div style="position:relative; width:100%; height:100%; background:#0d1117;">
            ...
            <iframe id="windyCloudIframe" src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=clouds&product=gfs&level=surface&lat=41.0803&lon=-105.5075" style="width:100%; height:100%; border:none;"></iframe>
        </div>
    `);
...
layout.registerComponent('nwsAlerts', function(container) {
    ...
    container.on('open', fetchWyomingAlerts);
});
...
layout.registerComponent('hydrologyFeed', function(container) {
    ...
    container.on('open', fetchWyomingHydrology);
});
...
function fetchWyomingAlerts() {
    const container = $('#alerts-container');
    console.log("Starting Wyoming alerts fetch...");
    
    // Use the NWS alerts API with a broad Wyoming search
    // Query all alerts and filter for WY
    fetch('https://api.weather.gov/alerts/active')
        ...
        .then(data => {
            ...
            // Filter for Wyoming alerts only
            let wyAlerts = allAlerts.filter(f => {
                const areaDesc = f.properties.areaDesc || '';
                const properties = f.properties;
                
                // Check if alert applies to WY
                return areaDesc.includes('WY') || 
                       areaDesc.includes('Wyoming') ||
                       (properties.areaDesc && properties.areaDesc.toLowerCase().includes('wyoming'));
            });
            
            console.log("Wyoming alerts found:", wyAlerts.length);
            wyAlerts.forEach(alert => {
            ...
            if (wyAlerts.length > 0) {
                html += `<div style="margin-bottom:10px; padding-bottom:8px; border-bottom:1px solid #30363d;">
                    <div style="font-size:0.75rem; color:#ffcc00; font-weight:bold; margin-bottom:5px;">ACTIVE ALERTS - ${wyAlerts.length}</div>`;
                ...
                wyAlerts.forEach(f => {
                    ...
                    const criticalEvents = ['Tornado Warning', 'Severe Thunderstorm Warning', 'Flash Flood Warning', 'Winter Storm Warning', 'Flood Warning', 'High Wind Warning', 'Blizzard Warning', 'Red Flag Warning'];
                ...
                html = "<span style='color:#00ff55; font-size:0.8rem;'>✓ SYSTEM CLEAN: NO ACTIVE ALERTS FOR WYOMING</span>";
            }
        ...
        .catch(err => {
            console.error("Wyoming alerts fetch error:", err);
...
function openAlertDetails(id) {
    ...
    openFloatingModal(`WYOMING ALERT DETAILS`, body);
}
            ...
            let html = `
                <div style="margin-bottom:8px; padding-bottom:6px; border-bottom:1px dashed #30363d;">
                    <div style="font-size:0.85rem; color:#fff;">
                        <i class="fa-solid fa-satellite-dish"></i> CHEROKEE PARK RD / TIE SIDING, WY (82084)
                        ...
                        tooltip: {
                            callbacks: {
                                afterLabel: function (context) {
                                    const pct = contained[context.dataIndex];
                                    return (pct !== null && pct !== undefined) ? `Contained: ${pct}%` : 'Containment: N/A';
                                }
                            }
                        }
                    },
                    scales: {
                        x: { grid: { color: '#21262d' }, title: { display: true, text: 'Acres' } },
                        y: { grid: { color: '#21262d' }, ticks: { autoSkip: false, font: { size: 9 } } }
                    }
                }
            });
...
function fetchWyomingHydrology() {
